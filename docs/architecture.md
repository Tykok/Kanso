...
| 6 | Writing an unknown `select` option silently creates it, so the status vocabulary drifts. | The vocabulary is the *team's*, and closed all the same: `V41` replaced `tickets_status_chk` with `tickets_status_fk` onto `team_statuses`, so a key that team has not declared is refused by Postgres. An unknown value coming back from Notion is logged and ignored, never adopted — a mirror does not get to invent a status any more than it did when the six were a `CHECK`. |
...
| 13 | An imported select option has no reason to be a word Kanso knows: `Terminé`, `Bloqué`, `P0`. | Every option of a mapped column is on screen with the Kanso value it will take. Keeping that true takes two things, because `MappedPageReader` still matches an option's own label where the mapping says nothing: `ImportSchema` sends the option table for **every** candidate column rather than only the one its name suggestion matched, and the third step seeds that table whenever a column is picked and prints the label match — not the field default — for whatever is left on "— default —". The options that really do take the field's default are **named** rather than counted: a number tells the reader something was guessed, a list tells them what. An option Kanso has no word for takes that default, never a seventh status nothing else understands. |
