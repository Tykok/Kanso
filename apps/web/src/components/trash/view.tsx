...
export function TrashView() {
  ...
  return (
    <>
      <div className="flex items-center gap-2.5 rounded-t-panel bg-background px-5 py-3 text-12 text-faint">
        <span>Trash</span>
      ...
      <div role="tablist" aria-label="Trash and archives" className="flex gap-5 px-5 pt-4 pb-3">
        {(["trash", "archives"] as const).map((id) => (
          <button
            key={id}
            type="button"
            role="tab"
            ...
            onClick={() => {
              setTab(id);
              // The two tabs hold disjoint sets, so a cursor from one names nothing in the
              // other; clearing it lets the fallback above pick the new tab's first row.
      ...
      <div role="tabpanel" className="flex min-h-0 flex-col gap-row px-5 pb-5">
